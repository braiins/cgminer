...
bool spi_poll_result(struct A1_chain *pChain, uint8_t cmd, uint8_t chip_id, uint8_t *buff, int len)
{
	...
	for(tmp_len = 0; tmp_len < tx_len; tmp_len += 2)
	{
		...
		if(spi_rx[0] == cmd)
		{
			if(!spi_read_data(ctx, spi_rx + 2, len))
			{
				applog(LOG_WARNING, "poll-result: transfer fail !");
				return false;
			}
...
bool inno_cmd_write_reg(struct A1_chain *pChain, uint8_t chip_id, uint8_t *reg)
{
	...
	memset(spi_tx, 0, sizeof(spi_tx));
 	...
 	spi_tx[1] = chip_id;
 	memcpy(spi_tx+2, reg, REG_LENGTH);
	...
	clc_crc = CRC16_2(tmp_buf, REG_LENGTH+2);

	spi_tx[REG_LENGTH+2] = (uint8_t)(clc_crc >> 8);
	spi_tx[REG_LENGTH+3] = (uint8_t)(clc_crc);
...
bool inno_cmd_read_reg(struct A1_chain *pChain, uint8_t chip_id, uint8_t *reg)
{
	...
	memset(spi_tx, 0, sizeof(spi_tx));
	if(!spi_send_command(pChain, CMD_READ_REG, chip_id, spi_tx, REG_LENGTH))
	{
		return false;
	}
	...
	for(i = 0; i < tx_len; i = i + 2)
	{
		...
		{
			if(!spi_read_data(ctx, spi_rx + 2, REG_LENGTH))
...
bool inno_cmd_read_result(struct A1_chain *pChain, uint8_t chip_id, uint8_t *res)
{
	...
	memset(spi_tx, 0, sizeof(spi_tx));
	if(!spi_send_command(pChain, CMD_READ_RESULT, chip_id, spi_tx, ASIC_RESULT_LEN))
	{
		return false;
	}
	...
	if(!spi_read_data(ctx, spi_rx + 2, READ_RESULT_LEN))
	{
		return false;
	}
	hexdump("result: RX", spi_rx + 2, READ_RESULT_LEN);
...
/*
	...
	res_crc = (spi_rx[READ_RESULT_LEN] << 8) + (spi_rx[READ_RESULT_LEN+1] << 0);
	...
	{
		printf("crc error clc=0x%4x, res=0x%4x \r\n", clc_crc, res_crc);
		return false;
	}
*/
...
uint8_t inno_cmd_isBusy(struct A1_chain *pChain, uint8_t chip_id)
{
    ...
    if(!spi_send_command(pChain, CMD_READ_REG, chip_id, spi_tx, REG_LENGTH))
    {
    	applog(LOG_WARNING, "send read reg command fail !");
    	return -1;
    }
...
/*
	memset(spi_rx, 0, sizeof(spi_rx));
	poll_len = JOB_LENGTH + 4 * chip_id - 1;
	for(i = 0; i < poll_len; i += 2)
	{
		...
		hexdump("read: RX", spi_rx, 2);
		if((spi_rx[0] == job[0]) && (spi_rx[1] == job[1]))
		{
			applog(LOG_DEBUG, "write job ok");
			return true;					
		}
	}
